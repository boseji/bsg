// time.go - Part of the `gen` Package
//
//     ॐ भूर्भुवः स्वः
...
// BST provides the current Bharat (India) Standard Time
func BST() time.Time {
	// No need to check its always correct
	loc, _ := time.LoadLocation("Asia/Kolkata")
	t := time.Now()
	return t.In(loc)
}

// ToBST converts the supplied to Bharat (India) Standard Time
func ToBST(t time.Time) time.Time {
	// No need to check its always correct
	loc, _ := time.LoadLocation("Asia/Kolkata")
	return t.In(loc)
}
